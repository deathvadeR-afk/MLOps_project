...
class ModelPusher:
    ...
    def initiate_model_pusher(self) -> ModelPusherArtifact:
        ...
        try:
            print("------------------------------------------------------------------------------------------------")
            logging.info("Uploading artifacts folder to MinIO bucket")
            
            logging.info("Uploading new model to MinIO bucket....")
            self.proj1_estimator.save_model(from_file=self.model_evaluation_artifact.trained_model_path)
            model_pusher_artifact = ModelPusherArtifact(bucket_name=self.model_pusher_config.bucket_name,
                                                        s3_model_path=self.model_pusher_config.s3_model_key_path)

            logging.info("Uploaded artifacts folder to MinIO bucket")
...
# For backward compatibility with AWS naming
class ModelPusherAWS(ModelPusher):
    pass